...
def main(start_url: str, pages: int | None, out_csv: str, delay_sec: float = 1.0, until_empty: bool = False):
    ...
    # Keep title as column B; add formula columns for translation and image preview.
    fieldnames = [
        "item_id",      # A
        "title",        # B
        "price_jpy",    # C
    ...
    with out_path.open("w", newline="", encoding="utf-8-sig") as f:
        ...
        while True:
            ...
            for row in items:
                # Translate title (Microsoft 365)
                row["title_en"] = f'=TRANSLATE(B{excel_row},"ja","en")'
                # Image preview using URL in column F (image_url)
                # Works in Microsoft 365 and Google Sheets
                row["image_preview"] = f'=IF(LEN(F{excel_row}),IMAGE(F{excel_row},"",3,160,160),"")'
